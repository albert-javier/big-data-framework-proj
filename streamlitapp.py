...
# Load models
cnn_model = load_model('asl_recognition_model.h5')
...
# Streamlit UI
st.title("American Sign Language Recognition Application")
...
if uploaded_file is not None:
    ...
    st.code(predicted_label)

    if model_choice == "CNN (MobileNetV2)":
        st.subheader("Top 5 Class Probabilities:")
        for label, prob in top_5_predictions:
            st.code(f"{label}: { (prob*100):.2f} %")

# Footer
st.write("This app supports CNN (MobileNetV2) model for ASL recognition.")
